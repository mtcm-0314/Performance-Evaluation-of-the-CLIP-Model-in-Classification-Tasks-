...
class ClipModel(object):
    ...
    #def finetune(self, dataloader, labels, val_loader, testloader, optimizer, nepochs, weight_decay):
        ...
        self.model.load_state_dict(torch.load('./SavedModels/NoisyDA/lr=1e-5,SGD/0.2' + '/NoisyDABest.pth'))
        eval_acc, pre, rec, f1, tpr, fpr, auc, net_benefits = self.evaluate(testloader, labels)
        ...
        SavedNetBenefits = np.array(net_benefits, dtype=float)
        ...
        np.savetxt('./Results/NoisyDA/ViTDCA' + '/TestingNetBenefits.csv', SavedNetBenefits, delimiter=',', fmt='%.6f')
        return best_acc
    ...
    def evaluate(self, dataloader, labels):
        ...
        auc = [auc]
        return acc, pre, rec, f1, tpr, fpr, auc, net_benefits
